...
function useTargetNetwork() {
    ...
    (0, react_1.useEffect)(() => {
        const newSelectedNetwork = scaffold_config_1.default.targetNetworks.find(targetNetwork => targetNetwork.id === chain?.id);
        if (newSelectedNetwork && newSelectedNetwork.id !== targetNetwork.id) {
            setTargetNetwork({ ...newSelectedNetwork, ...helper_1.NETWORKS_EXTRA_DATA[newSelectedNetwork.id] });
        }
    }, [chain?.id, setTargetNetwork, targetNetwork.id]);
    return (0, react_1.useMemo)(() => ({ targetNetwork }), [targetNetwork]);
}
