...
const useVotingWagmi = (parameters) => {
    ...
    const { chainId, ethersReadonlyProvider, ethersSigner, accounts, isConnected } = (0, useWagmiEthers_1.useWagmiEthers)(initialMockChains);
    const queryClient = (0, react_query_1.useQueryClient)();
    const [refreshKey, setRefreshKey] = (0, react_1.useState)(0);
    const [selectedPollId, setSelectedPollId] = (0, react_1.useState)(undefined);
    const [forceUpdate, setForceUpdate] = (0, react_1.useState)(0);
    const allowedChainId = typeof chainId === "number" ? chainId : undefined;
    const { data: votingContract, isLoading: isContractLoading } = (0, helper_1.useDeployedContractInfo)({ contractName: "Voting", chainId: allowedChainId });
    // Fallback: Get contract from deployedContracts directly if verification fails
    const contractsData = (0, react_1.useMemo)(() => {
        try {
            ...
            console.log(`[VotingHook] Contract details:`, contractNames.map(name => ({
                name,
                hasAddress: !!chainContracts[name]?.address,
                address: chainContracts[name]?.address,
            })));
            // Direct access - check if Voting exists
            console.log(`[VotingHook] Checking for Voting contract...`);
            console.log(`[VotingHook] chainContracts.Voting:`, chainContracts.Voting);
            ...
            const votingContract = chainContracts.Voting ||
            ...
            if (!votingContract) {
                ...
                if (votingLike) {
                    ...
                    const found = chainContracts[votingLike];
                    if (found) {
            ...
            console.error("[VotingHook] Error stack:", e.stack);
            return undefined;
        }
    }, [chainId]);
    ...
    const getContract = (mode) => {
        if (!hasContract || !finalVotingContract)
            return undefined;
        const providerOrSigner = mode === "read" ? ethersReadonlyProvider : ethersSigner;
        if (!providerOrSigner)
            return undefined;
        return new ethers_1.ethers.Contract(finalVotingContract.address, finalVotingContract.abi, providerOrSigner);
    };
    // Read poll count
    const pollCountResult = (0, wagmi_1.useReadContract)({
        address: (hasContract && finalVotingContract ? finalVotingContract.address : undefined),
        abi: (hasContract && finalVotingContract ? finalVotingContract.abi : undefined),
    ...
    const pollCount = pollCountResult.data ?? 0n;
    ...
    const pollInfoResult = (0, wagmi_1.useReadContract)({
        address: (hasContract && finalVotingContract && selectedPollId !== undefined ? finalVotingContract.address : undefined),
        abi: (hasContract && finalVotingContract ? finalVotingContract.abi : undefined),
    ...
    const pollInfo = (0, react_1.useMemo)(() => {
        if (!pollInfoResult.data)
            return undefined;
        const data = pollInfoResult.data;
        return {
            title: data[0],
            description: data[1],
            active: data[2],
            optionCount: Number(data[3]),
            totalVotes: Number(data[4]),
        };
    }, [pollInfoResult.data]);
    // Read admin address
    const adminResult = (0, wagmi_1.useReadContract)({
        address: (hasContract && finalVotingContract ? finalVotingContract.address : undefined),
        abi: (hasContract && finalVotingContract ? finalVotingContract.abi : undefined),
        functionName: "admin",
        query: { enabled: Boolean(hasContract && hasProvider && finalVotingContract), refetchOnWindowFocus: false },
    });
    const admin = adminResult.data ?? undefined;
    const isAdmin = (0, react_1.useMemo)(() => {
        if (!admin || !accounts || accounts.length === 0)
            return false;
        return admin.toLowerCase() === accounts[0].toLowerCase();
    }, [admin, accounts]);
    // Check if user has voted
    const hasVotedResult = (0, wagmi_1.useReadContract)({
        address: (hasContract && finalVotingContract && selectedPollId !== undefined && accounts && accounts[0]
            ? finalVotingContract.address : undefined),
        abi: (hasContract && finalVotingContract ? finalVotingContract.abi : undefined),
    ...
    const userHasVoted = hasVotedResult.data;
    // Get encrypted vote counts for all options
    const [encryptedCounts, setEncryptedCounts] = (0, react_1.useState)({});
    const [decryptedCounts, setDecryptedCounts] = (0, react_1.useState)({});
    const [optionDescriptions, setOptionDescriptions] = (0, react_1.useState)({});
    // Decrypt vote counts
    const requests = (0, react_1.useMemo)(() => {
        if (!hasContract || !finalVotingContract || !pollInfo || selectedPollId === undefined)
            return undefined;
        const reqs = [];
        for (let i = 0; i < pollInfo.optionCount; i++) {
            const handle = encryptedCounts[i];
            if (handle && handle !== ethers_1.ethers.ZeroHash) {
    ...
    const { canDecrypt, decrypt, isDecrypting, message: decryptMsg, results } = (0, _fhevm_sdk_1.useFHEDecrypt)({
        instance,
    ...
    (0, react_1.useEffect)(() => {
        if (!results || !pollInfo?.optionCount)
            return;
    ...
    const refreshContractData = (0, react_1.useCallback)(async () => {
        if (!hasContract || !hasProvider || !finalVotingContract?.address)
            return false;
        try {
            ...
            if (selectedPollId !== undefined) {
                refetchPromises.push(pollInfoResult.refetch().catch(err => {
    ...
    // Encrypt vote
    const { encryptWith } = (0, _fhevm_sdk_1.useFHEEncryption)({ instance, ethersSigner: ethersSigner, contractAddress: finalVotingContract?.address });
    ...
    const getEncryptionMethodFor = (functionName) => {
        ...
        if (!functionAbi.inputs || functionAbi.inputs.length < 3)
            return { method: undefined, error: `Invalid inputs for ${functionName}` };
        ...
        return { method: (0, _fhevm_sdk_1.getEncryptionMethod)(voteInput.internalType), error: undefined };
    };
    const createPoll = (0, react_1.useCallback)(async (title, description, options) => {
    ...
    const castVote = (0, react_1.useCallback)(async (pollId, optionIndex) => {
        if (isProcessing || !hasContract || !instance || !hasSigner || userHasVoted)
            return;
    ...
    const loadEncryptedCounts = (0, react_1.useCallback)(async () => {
        if (!hasContract || !pollInfo || selectedPollId === undefined)
            return;
        const read = getContract("read");
        if (!read)
            return;
    ...
    const allowAdminToDecrypt = (0, react_1.useCallback)(async (pollId, optionIndex) => {
        if (!isAdmin) {
...
exports.useVotingWagmi = useVotingWagmi;
